...
class RNCN:
    ...
    def initParams(self):
        ...
        self.W1 = 0.01*np.random.randn(self.middleDim,2*self.wvecDim)
        self.b1 = np.zeros((self.middleDim))

        # Hidden activation weights for layer 2
        self.W2 = 0.01*np.random.randn(self.wvecDim, 2*self.wvecDim + self.middleDim)
        self.b2 = np.zeros((self.wvecDim))
        ...
        self.db1 = np.empty((self.middleDim))
        
        self.dW2 = np.empty(self.W2.shape)
        self.db2 = np.empty((self.wvecDim))
	...
	node.fprop = True;
	if node.isLeaf:
		node.hActs2 = self.L[:, node.word];
	else:
		cost_l , total_l = self.forwardProp(node.left, correct, guess);
		cost_r , total_r = self.forwardProp(node.right, correct, guess);
		z1 = self.W1.dot(np.hstack([node.left.hActs2, node.right.hActs2])) + self.b1;
		node.hActs1 = z1 * (z1 > 0);
		z2 = self.W2.dot(np.hstack([node.left.hActs2, node.right.hActs2, node.hActs1])) + self.b2;
		node.hActs2 = z2 * (z2 > 0);
	...
	self.db2 += deltas;
	self.dW2 += np.outer(deltas,\
			np.hstack([node.left.hActs2, node.right.hActs2, node.hActs1]));
	deltas = np.dot(self.W2.T, deltas)

	deltas_h1 = deltas[2*self.wvecDim:].copy();
	deltas_h1 *= (node.hActs1 >0)

	self.db1 += deltas_h1
	self.dW1 += np.outer(deltas_h1, np.hstack([node.left.hActs2, node.right.hActs2]));
	
	deltas_h1 = np.dot(self.W1.T, deltas_h1);
	self.backProp(node.left, deltas_h1[:self.wvecDim] + deltas[:self.wvecDim])
	self.backProp(node.right, deltas_h1[self.wvecDim:] + deltas[self.wvecDim:2*self.wvecDim])
...
if __name__ == '__main__':
    ...
    rnn = RNCN(wvecDim,middleDim,outputDim,numW,mbSize=4)
    rnn.initParams()
